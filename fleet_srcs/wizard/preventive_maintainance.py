...
class ForcastDemand(models.Model):
    _name = 'preventive.maintainance.report'
    ...
    def print_report(self):
        for report in self:
            ...
            excel_sheet = workbook.add_worksheet('Preventive Maintainance')
            report_title = 'Preventive Maintainance For SRCS Fleets '
            ...
            col += 1
            excel_sheet.set_column(col, col, 25)
            excel_sheet.write(row, col, 'Licence Plate', header_format)
            col += 1
            excel_sheet.set_column(col, col, 20)
            excel_sheet.write(row, col, 'Chassis No', header_format)
            col += 1
            excel_sheet.set_column(col, col, 20)
            excel_sheet.write(row, col, 'Odometer', header_format)
            col += 1
            excel_sheet.set_column(col, col, 20)
            excel_sheet.write(row, col, 'Required Service', header_format)
            col += 1
            ...
            for fleet in fleets:
                if fleet.services_list:
                    ...
                    col += 1
                    excel_sheet.write(row, col, fleet.odometer, format)
                    col += 1
                    excel_sheet.write(row, col, serv.service.name, format)
                    col += 1
                    col = 0
                    row += 1
            ...
            fp.close()
            wizardmodel = self.env['preventive.maintainance.report.excel']
            res_id = wizardmodel.create({'name': file_name, 'file_download': file_download})
            return {
                'name': 'Files to Download',
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'preventive.maintainance.report.excel',
...
class Preventive_Maintainance_Report_Excel(models.TransientModel):
    _name = 'preventive.maintainance.report.excel'
