...
class HrCustody(models.Model):
    ...
    def renew_approve(self):
        for custody in self.env['hr.custody'].search([('custody_name', '=', self.custody_name.id)]):
            if custody.state == "asset_manager":
    ...
    def asset_manager_approve(self):
        for custody in self.env['hr.custody'].search([('custody_name', '=', self.custody_name.id)]):
            if custody.state == "asset_manager":
                raise UserError(_("Custody is not available now"))
    ...
    purpose = fields.Char(string='Reason', track_visibility='always', required=True, readonly=True, help="Reason",
                          states={'draft': [('readonly', False)]})
    custody_name = fields.Many2one('account.asset', string='Property', required=True, readonly=True,
                                   help="Property name",
                                   states={'draft': [('readonly', False)]}
                                   )
    ...
    state = fields.Selection([('draft', 'Draft'), ('to_approve', 'Waiting For Approval'), ('approved', 'Approved'),('asset_manager','Asset Manager Approve'),
