...
class HRContract(models.Model):
    ...
    # rule_ids = fields.One2many('salary.rule.compute','contract_id',string="Salary Information")
...
class SalaryPlan(models.Model):
    ...
    coverage_in_usd = fields.Float(string="Coverage In USD",compute="_compute_coverage_in_usd")
    ...
    def _compute_coverage_in_usd(self):
        for rec in self:
            rec.coverage_in_usd = (self.contract_id.wage * rec.percentage_of_covering) / 100
...
class HRPayslipHelal(models.Model):
    ...
    #     print("\n\n\n\n\n\n\n")
    #     print("*******************_prepare_line_values")
    #     res = super(HRPayslipHelal,self)._prepare_line_values(line, account_id, date, debit, credit)
    #     if line.slip_id.contract_id.salary_plan:
    #         for plan in line.slip_id.contract_id.salary_plan:
    #             res['analytic_account_id'] = plan.covered_by.id
    #     else:
    #         res['analytic_account_id'] = line.salary_rule_id.analytic_account_id.id or line.slip_id.contract_id.analytic_account_id.id
    #     return res        
...
#     _name = 'salary.rule.compute'


#     contract_id = fields.Many2one('hr.contract')
#     struct_id = fields.Many2one('hr.payroll.structure',related="contract_id.struct_id")
#     basic = fields.Float(string="Basic",compute="_compute_basic")

    
#     @api.depends('struct_id')
#     def _compute_basic(self):
#         if self.struct_id.id == self.env.ref('hr_contract_srcs.structure_permanent_contracts').id:
#             self.basic = (self.contract_id.wage * 0.6 ) * self.contract_id.rate
