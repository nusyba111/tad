        ...
        if self.move_type == 'in_invoice':
            for rec in self.invoice_line_ids:
                ...
                budget_line = self.env['crossovered.budget.lines'].search([('date_from','<=',rec.date),('date_to','>=',rec.date),('location_id','=',rec.location_id.id),
                                    ...
                                    ('crossovered_budget_id.donor_id','=',rec.partner_id.id)])
...
class SrcBudgetLine(models.Model):
    ...
    total_budget_SDG = fields.Monetary(compute='_compute_total_budget', string='Total budget ',currency_field='currency_id',store=True)
    ...
    def _compute_total_budget(self):
        for rec in self:
            if rec.unit_of_measure and rec.quantity and rec.frequency and rec.unit_cost:
                ...
                # rec.total_budget_SDG = rec.total_budget / rec.currency_budget_line.rate
            else:
                rec.total_budget = 0
                ...
                if move_line:
                    for line in move_line:
                        ...
                        if line.move_id.move_type == 'in_invoice':
                            amount += line.amount_from_conversion
                            res.practical_amount_bu_currency = -(amount)
                            print('__________________________practical',res.practical_amount_bu_currency,line.move_id.move_type)
    ...
    @api.depends('practical_amount','total_budget_SDG')
    def _compute_balance_SDG(self):
        for rec in self:
            rec.balance_SDG = rec.total_budget_SDG - rec.practical_amount
...
class Conversion(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', string='Currency',related="budget_id.currency_id")
    date = fields.Date('Date')
    rate = fields.Float('Rate',related="currency_id.rate",required=True)
