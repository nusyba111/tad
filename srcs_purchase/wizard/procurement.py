...
class Procurment(models.Model):
    ...
    #     string="Year",
    #     default="2022",  # as a default value it would be 2022
    # )
    ...
    def print_report(self):
        for report in self:
            ...
            fp.close()
            wizardmodel = self.env['procurement.report.excel']
            res_id = wizardmodel.create({'name': file_name, 'file_download': file_download})
            return {
                'name': 'Files to Download',
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'procurement.report.excel',
...
class Fuel_Tracking(models.TransientModel):
    _name = 'procurement.report.excel'
